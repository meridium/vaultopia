(function ($, ns) {
	...
	*/
    ...
    buttonBase.prototype = {
        ...
        loadClient: function (callback, scope) {
            ...
            this.loadjQuery(function (disposejQueryCallback) {
                this.loadScript(this.editor.settings.imagevault_imageVaultEPiServerCmsBaseUrl + "Scripts/imagevault-client-script/scripts/client.min.js", isIvClientLoaded, function () {
                    disposejQueryCallback();
                    callback.call(scope || window);
                }, this);
            }, this);
        ...
        loadFancyBox: function (callback, scope) {
            ...
            this.loadjQuery(function (disposejQueryCallback) {
                ...
                if (window.iv_jQuery.browser.msie) {
	...
	*/
    ...
    $.extend(editButton.prototype, {
            ...
            this.getClient(function (client) {
                ...
                client.json("MediaService/Find", json, function (x) {
                    ...
                    self.loadPropertyMediaCommon(function () {
                        ...
                        self.getEffectsFromFormat(media.SelectedMedia.MediaFormatId, media, function (effects) {
                            ...
                            self.loadFancyBox(function () {
                                ...
                                self.spinner = null;
                                pmc.openEditor(mediaCache, mediaReference, undefined, true, false, null, false, self.format);
                            });
                        });
                    });
                });
        ...
        getEffectsFromFormat: function (formatId, media, callback) {
            var self = this;
            this.getClient(function (client) {
                var parameters = {
                ...
                client.json("MediaFormatService/Find", json, function (x) {
                    if (x && x.length) {
                        self.format = x[0];
                        var effects = self.format.Effects || [];

                        // If no effects on format get a mediareference with set height and with
                        if (effects.length === 0) {
                            var width = media.SelectedMedia.Width;
                            var height = media.SelectedMedia.Height;

                            // Use Scale to fit if no aspect ratio is set but both width and height on format
                            if (self.format.Width && self.format.Height && !self.format.AspectRatio) {
                                width = Math.min(self.format.Width, media.OriginalMedia.Width);
                                height = Math.min(self.format.Height, media.OriginalMedia.Height);
                            }

                            effects.push(
                            {
                                $type: "ImageVault.Common.Data.Effects.CropEffect,ImageVault.Common",
                                Width: width,
                                Height: height
                            });
    ...
    tinymce.create('tinymce.plugins.ImageVaultPlugin', {
        ...
        init: function (ed, url) {
            ...
            ////and video tags
            //if (ed.settings.extended_valid_elements.indexOf("video[") < 0) {
            //    ed.settings.extended_valid_elements = ed.settings.extended_valid_elements + ",video[*]";
            //}
            ////and source tags
            //if (ed.settings.extended_valid_elements.indexOf("source[") < 0) {
            //    ed.settings.extended_valid_elements = ed.settings.extended_valid_elements + ",source[*]";
            //}
            ////and img tags
            //if (ed.settings.extended_valid_elements.indexOf("img[") < 0) {
            //    ed.settings.extended_valid_elements = ed.settings.extended_valid_elements + ",img[*]";
            //}
