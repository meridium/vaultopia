...
Import-Module "$EPiServerPackagingScripts\Remove-Assembly.psm1"
Import-Module "$EPiServerPackagingScripts\Remove-AddOnFolders.psm1"

# Removes the given EPiServer AddOn (a protected module), its assemblies (from modulesbin), the contents (from the found modules folder) and removes the package entry from package.config
# $packageName is the name of the installed package/module 
# $projectPath is the path to the root of site and is optional (if not given then will get it from Get-Project cmdlet)
# $listOfAssembliesToBeRemoved is array of assemblies inside modulesbin which must be removed
Function Remove-EPiServerAddOn ($packageName, $projectPath, $listOfAssembliesToBeRemoved)
{  
    ...
    {
        $projectFullName = (Get-Project).FullName 
    ...
    if($projectPath -eq $null -or $projectPath -eq "")
    {                           
        throw (New-Object ArgumentException("Project path can't be found. So can't delete AddOn - $packageName"))
    }			  

    # get webconfig  
    $elementsToBeLoaded = (,"episerver.framework")
    ...
    {
        throw (New-Object ArgumentException("Protected modules path can't be found. Check the WebConfig or related connected config file to see if protected modules location is given"))
    }	   
    ...
    if(!(IsPackageFoundInConfig $packagesConfigPath $packageName) -and !(IsPackageFoundInConfig $disablePackagesConfigPath $packageName)) 
    {
        return 
    }
    ...
    try 
    {
        if($listOfAssembliesToBeRemoved -ne $null) 
        {
            foreach($assemblyName in $listOfAssembliesToBeRemoved) 
            {
                if(!($assemblyName -eq $null -or $assemblyName -eq "")) 
                {
                    Remove-Assembly $config $projectPath $assemblyName
                }
            }    
        }       
    ...
    {
        # Show a message box explaining that the package can't be removed completely
        $ErrorMessage = $_.Exception.Message
        [System.Windows.Forms.MessageBox]::Show("The package installer was unable to delete the AddOn assembly. You've to remove this assembly manually. The error is:- $ErrorMessage") | Out-Null
    }

    try 
    {
        #removes all the relevant folders 
        Remove-AddOnFolders $config $projectPath $packageName 
    }
    catch [Exception]
    {
        # Show a message box explaining that the package can't be removed completely
        $ErrorMessage = $_.Exception.Message
        [System.Windows.Forms.MessageBox]::Show("The package installer was unable to delete the AddOn folder. You've to remove this folder manually. The error is:- $ErrorMessage") | Out-Null
    }  
...
Export-ModuleMember -Function Remove-EPiServerAddOn
# SIG # Begin signature block
# MIIXnAYJKoZIhvcNAQcCoIIXjTCCF4kCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVgzG2xG9yhAXnFjXblSoij0i
# 1j6gghLKMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUHgHY0nSY
# GRwvp4ETWjuH5eTVpSgwDQYJKoZIhvcNAQEBBQAEggEAU0oaZ8UcKX6ft1ZFJhnE
# vgneJEfwzOLvfiVpsTaI+evq7qD5Vde4DYILYiI7bqlhdxOSS7VXYteiIMySUu9q
# xtatRXPZ3q7sQVfYvqzkwA1EueFSWBnViGXT4Tm922KBGakOf9XDk7SYHoDv64Q6
# 0enfk6dYqavZenoQQQtg5GDJQ1dhkeAL90V+U0VT5IBxM+R7cBf5hXb2T9CW5OUH
# LGuvXJWQAkDhUP3rLax3R6e3zeCAH0kcMoSDjOl7vJbp10SZgFCbx4qb5s1zCyzz
# P2drdfEWTOJTM+OYxz2JOb+fpuCxMOo5addjGFeqfGf5uBnXnxVjOGy8hQOQJYox
# gKGCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4xCzAJBgNVBAYTAlVT
...
# SIb3DQEJBTEPFw0xNjA1MTMxMzA4MDRaMCMGCSqGSIb3DQEJBDEWBBTnBGp6Wy9Y
# YMue3IZzFdfIga2DlDANBgkqhkiG9w0BAQEFAASCAQCCQSGLA62G5Xpb/gehaW3x
# o4oW8/VOJlmMwrWLr6hjAEdTSE1F8/O5GxkA/UQq5bMxZvhAhVIoidFBvEmLEi97
# 9g2n5TUVxBwOeGGZcHLz6CWourOqNfyQsCUQ2GguKBlH5GwW/TsCsDwkP3wJHp0i
# Xgd1JvamRetPpAh8QAWMuUO3x9EROpUAyf1lYC7OeyRU3Vd/ngexN4uxVeAOpJQ7
# 3vpK+SZznAjA6ckm6lLGEF+yE9bxoE1WC2nZlfjby9v0uJ+JuvA1EZXrMP1rKWKv
# kMxGLcy7MtKfv+ZHMXH0pr62gro4uz9letpUMNg8B7sOILqrBmr5LunYsQEgT8E8
