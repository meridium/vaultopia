function Get-ProtectedModulesPath($webConfig, $projectPath)
{
    if($webConfig -eq $null -or $projectPath -eq $null)
    {
        return $null
    }
	...
	# Resolve an absolut path for the modules location
	if(![System.IO.Path]::IsPathRooted($protectedModulesPath))
	{			
		$protectedModulesPath = Join-Path $projectPath $protectedModulesPath
	}
...
function Get-PublicModulesPath($webConfig, $projectPath)
{
    if($webConfig -eq $null -or $projectPath -eq $null)
    {
        return $null
    ...
    if ($packagingElement -ne $null)
    {
        $publicModulesPath = $packagingElement.GetAttribute("publicPath")
    }

    # If the $publicModulesPath modules path is null or empty then it isn't defined on the packing element and
    # we should fallback to the default path ([appDataPath]\Modules).
    if ($publicModulesPath -eq $null -or $publicModulesPath -eq "")
    {
        # Get the app data element
        $appDataElement = $webConfig.configuration."episerver.framework".appData
        ...
        if ($appDataElement -ne $null -and $appDataElement.HasAttribute("basePath"))
        {
            $publicModulesPath = (Join-Path $appDataElement.GetAttribute("basePath") "Modules")
        }
    }

	# Resolve an absolut path for the modules location
	if(![System.IO.Path]::IsPathRooted($publicModulesPath))
	{			
		$publicModulesPath = Join-Path $projectPath $publicModulesPath
	}

    return $publicModulesPath
}

function Get-ModulesRepositoryPath($webConfig, $projectPath)
{
    if($webConfig -eq $null -or $projectPath -eq $null)
    {
        return $null
    ...
    if ($appDataElement -ne $null -and $appDataElement.HasAttribute("basePath"))
    {
        $modulesRepositoryPath = (Join-Path $appDataElement.GetAttribute("basePath") "ModulesRepository")
    }

	# Resolve an absolut path for the modules repository path
	if(![System.IO.Path]::IsPathRooted($modulesRepositoryPath))
	{			
		$modulesRepositoryPath = Join-Path $projectPath $modulesRepositoryPath
	}
    return $modulesRepositoryPath
}

Export-ModuleMember -Function Get-ProtectedModulesPath, Get-PublicModulesPath, Get-ModulesRepositoryPath
# SIG # Begin signature block
# MIIXnAYJKoZIhvcNAQcCoIIXjTCCF4kCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUnQ1qCC+LaExsZSZc5K0vPEci
# NX2gghLKMIID7jCCA1egAwIBAgIQfpPr+3zGTlnqS5p31Ab8OzANBgkqhkiG9w0B
...
# BgEEAYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU5T7+lAbt
# DQff1lVGWm8y80Y8OFowDQYJKoZIhvcNAQEBBQAEggEAprpbWRBYmvDoygtYCHHv
# pbU4woYwQRQ81fB6GCYDmb9FXptILpXnRBHLDpFWgYUTalNM0jn3PpAlgHfCExaF
# 6Nm1zsKSf77r2JdinlpvKI8t1rebMjsFmSM+1gDoq+Ux0s8zQwrvms7LZ3yRcdt+
# d414gGbGQhdDXbcASdZIsUaMfDdV5ggTfXqvl5lgojglRJ7v0jcJKQtQbAaHoOoD
# mZWOI2EOFh+sMj1KNOdTguvh4PY7O36lI3vYF919f06EXClc9VG0nG1yz7n936YY
# vlpkBK39z1ci6c9GAVCiR/YCkKCzL7YRHYyaJ8Y4rtdtcIWRyW3dF93MYOqnQ93b
# X6GCAgswggIHBgkqhkiG9w0BCQYxggH4MIIB9AIBATByMF4xCzAJBgNVBAYTAlVT
...
# SIb3DQEJBTEPFw0xNjA1MTMxMzA3NTlaMCMGCSqGSIb3DQEJBDEWBBTpBKeYFaRo
# k9BBTgsTOnqUspYIFjANBgkqhkiG9w0BAQEFAASCAQAWq9hQyYp32MA++HodEHqQ
# VYVhDH7oiV6EFxQAf7p4SKzx/7bFz6ON/wBlexsShf6Fqrh85J9Bki/C+IQ4XEFy
# PFixciU4vLuozCso70H9eUo04zkG6Z6/rhEy3NXrA5CsQzD5/AAaWk8FUYAKwDEm
# VikgG3H8UxE/OpcZlbHdoiuRh5pwdMQoMxlb7JTUW0KZuS79TpcHGsTT/d1IAIYY
# FB5oJq9xOSLfoS1jW3nJ9QWcY8JyLJtA+kQv8PRw/dGKaaR9MTkJJC9P21q9chpj
# Ko82DNh5/lDGKCbe5b06U0GF/TQrwleRxQw787mtjzW1OMPTHzZFJzEY/mntNJxX
