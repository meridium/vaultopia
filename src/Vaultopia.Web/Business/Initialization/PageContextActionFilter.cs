...
namespace Vaultopia.Web.Business.Initialization {
    public class PageContextActionFilter : IResultFilter {
        ...
        public void OnResultExecuting(ResultExecutingContext filterContext) {
            object viewModel = filterContext.Controller.ViewData.Model;

            var model = viewModel as IPageViewModel<SitePageData>;
            if (model != null) {
                ContentReference currentContentLink = filterContext.RequestContext.GetContentLink();

                LayoutModel layoutModel = model.Layout ??
                                          _contextFactory.CreateLayoutModel(currentContentLink,
